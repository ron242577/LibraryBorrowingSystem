...
function ensureStudentDetailColumns($conn) {
    $columns = [
        ...
        'password'         => "ALTER TABLE students ADD COLUMN password VARCHAR(255) NOT NULL DEFAULT '' AFTER qr_code"
    ];
...
function createStudentRecord($conn, $student_no, $full_name, $student_group, $department, $year_level, $contact_number, $card_valid_until, $email, $password, &$generated_qr = '', &$error = '') {
    ...
    if (!in_array($year_level, $valid_year_levels, true)) {
        $error = 'Grade level must be Grade 7 to Grade 12.';
        return false;
    }
    if (in_array($year_level, $senior_high_grades, true)) {
        if (!in_array($department, $valid_departments, true)) {
            $error = 'A valid Senior High School strand is required for Grade 11 and Grade 12.';
            return false;
        }
    } else {
        $department = '';
    ...
    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $error = 'A valid email address is required for student verification.';
        return false;
    }
    $passwordErrors = passwordPolicyErrors($password);
    if ($password === '' || !empty($passwordErrors)) {
        $error = $password === '' ? 'Password is required.' : strongPasswordMessage($passwordErrors);
        return false;
    }

    try {
        $duplicate = $conn->prepare('SELECT student_id FROM students WHERE student_no = ? OR email = ? LIMIT 1');
        $duplicate->bind_param('ss', $student_no, $email);
        $duplicate->execute();
        $alreadyExists = $duplicate->get_result()->num_rows > 0;
        $duplicate->close();
        if ($alreadyExists) {
            $error = 'Student number or email is already registered.';
            return false;
        }
        ...
        $insert_stmt = $conn->prepare("INSERT INTO students (student_no, full_name, student_group, department, year_level, contact_number, card_valid_until, email, qr_code, password, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')");
        $insert_stmt->bind_param('ssssssssss', $student_no, $full_name, $student_group, $department, $year_level, $contact_number, $card_valid_until, $email, $student_qr_code, $hashed_password);
...
function rowsToStudentImportData($rows) {
    ...
    if (!in_array('full_name', $mapped_headers, true)) {
        ...
        $mapped_headers = [0 => 'full_name', 1 => 'student_no', 2 => 'student_group', 3 => 'department', 4 => 'year_level', 5 => 'contact_number', 6 => 'card_valid_until', 7 => 'email', 8 => 'password'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    $generated_qr = '';
    $error = '';
    if ($message_type !== 'error' && !hash_equals((string)($_POST['password'] ?? ''), (string)($_POST['confirm_password'] ?? ''))) {
        $message = 'Password confirmation does not match.';
        $message_type = 'error';
    }
    if ($message_type !== 'error' && createStudentRecord(
        $conn,
        $_POST['student_no'] ?? '',
        $_POST['full_name'] ?? '',
        $_POST['student_group'] ?? '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'import') {
    ...
    if ($message_type === 'error') {
        // Stop before processing the uploaded file.
    } else {
        try {
            $validatedUpload = validateSpreadsheetUpload($_FILES['students_file'] ?? []);
            $ext = $validatedUpload['extension'];
            $uploadPath = $validatedUpload['tmp_name'];
            if ($ext === 'xlsx') {
                $rows = parseStudentXlsx($uploadPath);
            } else {
                $rows = parseStudentCsv($uploadPath);
            }
            ...
            $safeImportErrors = ['XLSX import requires the PHP ZipArchive extension. You can upload CSV instead.', 'No student records found in the uploaded file.'];
            $message = in_array($e->getMessage(), $safeImportErrors, true) ? $e->getMessage() : 'Student import failed. Check that the file matches the current template and try again.';
            $message_type = 'error';
            logError('Student import error: ' . $e->getMessage());
        }
    }
}
...
<body>
        ...
        <div id="addStudentSection" class="add-student-section" style="display:none;">
            ...
            <div class="add-student-body" id="addStudentBody">
                <form method="POST" id="addStudentForm">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="inline_student_group">Section *</label>
                            <input type="text" id="inline_student_group" name="student_group" required placeholder="e.g. Rizal, 10-A">
                        </div>
                        <div class="form-group">
                            <label for="inline_year_level">Grade Level *</label>
                            <select id="inline_year_level" name="year_level" required>
                    ...
                    <div class="form-row">
                        <div class="form-group" id="inline_department_group" style="display:none;">
                            <label for="inline_department">Department / Strand *</label>
                            <select id="inline_department" name="department">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="inline_email" name="email" placeholder="e.g. student@gmail.com" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="inline_confirm_password">Confirm Password *</label>
                            <input type="password" id="inline_confirm_password" name="confirm_password" maxlength="128" autocomplete="new-password" required placeholder="Enter the same password again">
                        </div>
                    </div>
                    <div class="qr-info">
    ...
    function renderModal(data) {
        ...
        const txBadge = (status) => {
            const map = { borrowed: '#FBFDCB|#5C5F05', returned: '#EDF5DD|#344E15' };
        ...
        const isSeniorHigh = s.year_level === 'Grade 11' || s.year_level === 'Grade 12';
        const departmentHTML = isSeniorHigh ? `
                <div class="stu-info-item">
                    <div class="stu-info-label">Department / Strand</div>
        ...
        document.getElementById('modalBody').innerHTML = `
            ...
            <div class="stu-info-grid">
                ...
                <div class="stu-info-item">
                    <div class="stu-info-label">Section</div>
                    <div class="stu-info-value">${escHtml(s.student_group || 'N/A')}</div>
                </div>
                <div class="stu-info-item">
                    <div class="stu-info-label">Grade Level</div>
    ...
    function toggleInlineDepartment() {
        const grade = document.getElementById('inline_year_level');
        const group = document.getElementById('inline_department_group');
        const department = document.getElementById('inline_department');
        if (!grade || !group || !department) return;
        const isSeniorHigh = grade.value === 'Grade 11' || grade.value === 'Grade 12';
        group.style.display = isSeniorHigh ? 'flex' : 'none';
        department.required = isSeniorHigh;
        if (!isSeniorHigh) department.value = '';
    }

    document.addEventListener('DOMContentLoaded', function () {
        const grade = document.getElementById('inline_year_level');
        if (grade) {
            grade.addEventListener('change', toggleInlineDepartment);
            toggleInlineDepartment();
        }
    });
